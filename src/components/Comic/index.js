...
const Comic = ({ title, img, id, description }) => {
  ...
  const subdescription = description ? description.substr(0, 200)+'...' : 'No description available'

  return (
    <>
      <Link to={`/comic/${id}`} className="link">
        <div className="comic">
          <div className="img-comic" style={{ backgroundImage: `url(${img.path}.${img.extension})` }}>
          </div>
